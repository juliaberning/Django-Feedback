...
class UserProfile(models.Model):
    ...
    department = models.CharField(
        max_length=100, choices=DEPARTMENT_CHOICES, default="SE"
    )
...
class Review(models.Model):
    feedback_process = models.ForeignKey(
        FeedbackProcess, on_delete=models.CASCADE, related_name="review"
    )
    reviewer = models.ForeignKey(
        UserProfile, on_delete=models.CASCADE, null=True, related_name="reviews_given"
    )
    ...
    reviewee_growth_rating = models.IntegerField(
        blank=True, null=True, choices=GROWTH_CHOICES
    )
    reviewee_execution_rating = models.IntegerField(
        blank=True, null=True, choices=EXECUTION_CHOICES
    )
    reviewee_collaboration_rating = models.IntegerField(
        blank=True, null=True, choices=COLLABORATION_CHOICES
    )
